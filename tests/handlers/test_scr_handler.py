...
def mock_scrutin_adopte() -> MagicMock:
    scrutin = MagicMock()
    scrutin.ref = "456"
    scrutin.sort = "adopté"
...
@patch('builtins.open', mock_open(read_data='{}'))
def test_scr_handler_not_found(
    _mock_scrutin: MagicMock, 
    _mock_listdir: MagicMock, 
    code_ref: str) -> None:
    embed = scr_handler(code_ref)
...
def test_scr_handler_malformed_json(
    _mock_scrutin: MagicMock, 
    _mock_listdir: MagicMock, 
    code_ref: str) -> None:
    embed = scr_handler(code_ref)
